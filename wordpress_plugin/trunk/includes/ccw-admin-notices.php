...
if (!class_exists('CCW_Admin_Notices')) {
    ...
    class CCW_Admin_Notices
    {
        ...
        {
            if (isset($_POST['CCW_dismiss'])) {
                $dismiss_option = sanitize_text_field($_POST['CCW_dismiss']);
            ...
            foreach (explode(',', self::TYPES) as $type) {
                foreach ($this->admin_notices->{$type} as $admin_notice) {
                    ...
                    if (!get_option("CCW_dismissed_{$admin_notice->dismiss_option}") || strpos($screen->id, 'crypto-converter-widget') !== false) {
                        ?><div class="notice is-dismissible ccw-notice notice-<?php echo esc_attr($type); ?>"
                        data-dismiss-url="<?php echo esc_url($dismiss_url); ?>">

                            <div class="ccw-rate-notice-container">
                                <div class="logo-img">
                                    <img src="<?php echo esc_attr(CCW_URL . 'assets/admin/img/icon.svg'); ?>" style="width:96px">
                                </div>
                                <div>
